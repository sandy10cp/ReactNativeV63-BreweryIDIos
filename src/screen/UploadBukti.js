...
export class UploadBukti extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
            ...
            idorders: props.route.params.idorders,
            image: null,
            dataImage: null,
            fileName: '',
            tipeGambah: '',
    ...
    uploadBuktiTransfer = () => {
        ...
        const url = 'https://app.brewery-id.com/buyer/kirimbukti.asp';
        ...
        if (this.state.image == null) {
            ...
            formData.append('file', {
                ...
                // type: this.state.tipeGambah || 'jpg', // or photo.type
            ...
            fetch(url, {
                ...
                .then((result) => {
                    ...
                    if (result.ErrorCode == '0') {
                        ...
                        // this.props.navigation.replace('BottomTabs', { initialRoute: 'Activity' })
                    } else {
                        setTimeout(() => {
    ...
    fetchInfoTransfer() {
        const { idbuyer, auth, idcabang, idorders } = this.state
        const data = { aksi: 129, idbuyer, authkey: auth, idcabang, idorders }
        API.Fetch(data)
            .then((result) => {
                // console.log(result)
                if (result.ErrorCode == '0') {
                    ...
                    <View style={{ justifyContent: 'center', alignItems: 'center', marginBottom: 10 }}>
                        <View style={styles.infoTransfer}>
                            ...
                            <View style={{ flexDirection: 'row' }}>
                                <View style={{ width: '45%' }}>
                                    <Text style={{ fontSize: 20, fontWeight: '600' }}>Bank/OVO/GoPay</Text>
                                </View>
                                <View style={{ width: '55%' }}>
                                    <Text style={{ fontSize: 20, fontWeight: '600' }}>{infotransfer.NamaBank == null ? ': -' : ': ' + infotransfer.NamaBank}</Text>
                                </View>
                            </View>
                            <View style={{ flexDirection: 'row' }}>
                                <View style={{ width: '45%' }}>
                                    <Text style={{ fontSize: 20, fontWeight: '600' }}>Nama Pemilik</Text>
                                </View>
                                <View style={{ width: '55%' }}>
                                    <Text style={{ fontSize: 20, fontWeight: '600' }}>{infotransfer.NamaPemilik == null ? ': -' : ': ' + infotransfer.NamaPemilik}</Text>
                                </View>
                            </View>
                            <View style={{ flexDirection: 'row' }}>
                                <View style={{ width: '45%' }}>
                                    <Text style={{ fontSize: 20, fontWeight: '600' }}>No. Rekening</Text>
                                </View>
                                <View style={{ width: '55%', flexDirection: 'row' }}>
                                    <Text style={{ fontSize: 20, fontWeight: '600' }}>{infotransfer.Norek == null ? ': -' : ': ' + infotransfer.Norek}</Text>
                                    <TouchableOpacity
                            ...
                            <View style={{ flexDirection: 'row' }}>
                                <View style={{ width: '45%' }}>
                                    <Text style={{ fontSize: 20, fontWeight: '600' }}>Jumlah Transfer</Text>
                                </View>
                                <View style={{ width: '55%' }}>
                                    <Text style={{ fontSize: 20, fontWeight: '600' }}>{infotransfer.Jumlah == null ? ': -' : ': Rp ' + currencyFormat(Number(infotransfer.Jumlah))}</Text>
                                </View>
                            </View>
                        </View>
